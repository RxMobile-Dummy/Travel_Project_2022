...
class HotelDataSourceRepositoriesImpl implements HotelDataRourceRepositories {
  ...
  Future<Either<Failures, void>> postHotel(Hotel hotel) async {
    try {
      Response response =
          await dio.post('http://192.168.102.79:4000/hotel/addhotel',options: await createDioOptions(), data: hotel);
  ...
  Future<Either<Failures, List<HotelModels>>> getHotel() async {
    try {
      ...
      final response = await dio.get('http://192.168.102.79:4000/hotel',options: await createDioOptions());
      if (response.statusCode == 200) {
        print("success");
  ...
  Future<Either<Failures, void>> deleteHotel(String id) async{
    ...
    try {
      ...
      final response = await dio.delete(baseurl,options: await createDioOptions());
      if (response.statusCode == 200) {
           return Right(null);
        }
     else {
  ...
  Future<Either<Failures, HotelPutModel>> getHotelUpdate(String id) async{
    ...
    try {
      final response = await dio.get('http://192.168.102.79:4000/hotel/updatehoteldata/${int.parse(id)}',options: await createDioOptions());
      if (response.statusCode == 200) {
  ...
  Future<Either<Failures, void>> updateHotel(HotelPutModel hotelPutModel)async{
    try {
      final response = await dio.put('http://192.168.102.79:4000/hotel/updatehotel',options: await createDioOptions(),data: hotelPutModel.toJson());
